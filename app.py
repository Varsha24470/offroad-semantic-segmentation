...
with tab1:
    ...
    with left:
        ...
        if uploaded_file is not None:
            ...
            st.button("🧠 Run Segmentation", use_container_width=True)
            st.info("Live inference can be integrated in the next version.")
        else:
            st.markdown(
...
with tab3:
    ...
    with mc2:
        ...
        if PER_CLASS_METRICS.exists():
            st.image(str(PER_CLASS_METRICS), caption="Per-Class Performance", use_container_width=True)
        else:
